...
logical nl;
{
    ...
    const char *zkeye = "\x49\x61\x6E\x4C\x61\x6E\x63\x65\x54\x61\x79\x6C\x6F\x72\x4A\x72";
    char *c;
    long x;
    integer i;


    //ebcdic2ascii(zkey, strlen(zkey));
...
/* 						!SAID SOMETHING. */
...
#ifdef __DEBUG__
        printf("Args: n=%d y=%d z=%d nl=%d x + mrloc=%d\n", n, y, z, nl,(x + (long)rmsg_1.mrloc));
    ...
    while (1) {
        ...
        i ^= zkeye[x & 0xf] ^ (x & 0xff);
        //printf("X= %d, x & 0xf = %02X .. x & 0xff = %02X .. zkey[x & 0xf]=%02X (%c) .. i = %02X\n", x, (x & 0xf), (x & 0xff), zkey[x & 0xf], i);
        //i = ASCIItoEBCDIC(i);
        i = a2e[i];
        x = x + 1;
        //printf(" [%02X] ",i);
